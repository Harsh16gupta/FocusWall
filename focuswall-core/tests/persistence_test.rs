use focuswall_core::{Database, PolicyKind};
...
fn test_database_persistence_across_reopen() {
    ...
    // Session 1: Open and verify YouTube and adult content policies are seeded
    {
        let db = Database::open(&db_path).expect("opens db");
        let policies = db.get_active_policies().expect("gets policies");
        assert_eq!(policies.len(), 2);
        assert!(policies.iter().any(|p| p.name == "youtube" && p.kind == PolicyKind::System));
        assert!(policies.iter().any(|p| p.name == "adult_content" && p.kind == PolicyKind::System));
